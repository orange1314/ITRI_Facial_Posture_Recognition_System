...
def combine_data(data_path, output_file):
    """
    合併資料夾中的數據片段，並保存為一個 .npz 文件。
    
    參數:
    - data_path: str，包含數據的資料夾路徑。
    - output_file: str，保存合併後數據的 .npz 文件名稱。
    
    功能:
    - 遍歷資料夾中的所有子資料夾，讀取數據片段和標籤。
    - 將所有數據片段和標籤合併為 numpy 數組。
    - 保存合併後的數據和標籤到一個 .npz 文件。
    - 打印各類別的資料數量。
    """
    ...
    # 讀取資料夾中的所有子資料夾
    labels_folders = os.listdir(data_path)
    ...
    # 保存數據和標籤到 .npz 文件
    np.savez(output_file, data=data_slices, labels=labels)

    # 打印各類別的資料數量
    for label, count in label_counts.items():
        print(f"Category '{label}' has {count} slices.")

    print(f"Data and labels have been saved to {output_file} with {len(data_slices)} slices.")

if __name__ == "__main__":
    # 合併並保存訓練數據
    combine_data(train_path, 'data_combined.npz')
    # 合併並保存測試數據
    combine_data(test_path, 'test_data.npz')
